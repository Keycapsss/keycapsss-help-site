...
### I have bluetooth connection issue

Do a complete reset on both half's.

1. Download the follwowing files:
   - [settings_reset-seeeduino_xiao_ble-zmk.uf2](/firmware/settings_reset-seeeduino_xiao_ble-zmk.uf2){download} (factory reset the XIAO BLE)
   - [forager_left.rgbled_adapter-seeeduino_xiao_ble-zmk.uf2](/firmware/forager_left.rgbled_adapter-seeeduino_xiao_ble-zmk.uf2){download} (firmware for the left half)
   - [forager_right.rgbled_adapter-seeeduino_xiao_ble-zmk.uf2](/firmware/forager_right.rgbled_adapter-seeeduino_xiao_ble-zmk.uf2){download} (firmware for the right half)
2. Connect left half via usb to you computer
3. Double tap the reset button on the back twice
4. A mass storage device named XIAO BLE should appear on the host
5. Drag and drop `settings_reset-seeeduino_xiao_ble-zmk.uf2` file to the storage drive. The XIAO BLE will automatically reset. Wait 5 seconds.
6. Start from step 3, but not drag and drop the `forager_left.rgbled_adapter-seeeduino_xiao_ble-zmk.uf2` file.

Do the speps above again for the right half and use the `forager_right.rgbled_adapter-seeeduino_xiao_ble-zmk.uf2` file.
